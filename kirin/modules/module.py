...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        # print(111111, name)
        if name.startswith(('applications.addons.')) and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        setattr(applications.addons, addon_name, new_mod)
...
def initialize_sys_path():
    ...
    for ad in settings['ADDONS_PATH'].split(','):
    ...
    for ad in __import__('applications.addons').__path__:
...
def load_kirin_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('applications.addons.' + module_name)
